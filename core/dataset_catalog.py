...
def build_manifest_plan(config,rows,mouse_id:str,laser_nm:int|None=None,*,source_catalog:Path|None=None,validation_report:dict|None=None)->tuple[Path,bool]:
    ...
    for index,row in enumerate(selected):
        ...
        mask=base/"segmentation"/"mask.tif"; green=base/"preprocessing"/"green.tif"; red=base/"preprocessing"/"red.tif"; status="ready" if all(p.is_file() for p in (mask,red,green)) else "preprocessing_required"; ready &= status=="ready"
