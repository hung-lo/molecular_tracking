...
- `synthetic_stitch_threshold_sweep.csv`: deterministic replay of each saved synthetic batch through candidate tiering and global assignment under stitch distance and forward-margin profiles; it never selects a profile automatically.
- `manual_stitch_review_manifest.csv`: every non-rejected proposal; PNG review sampling only annotates selected rows with `review_sample_reason`; existing manual labels are restored by `stitch_edge_id` on overwrite.
- `review_panels/*.png` and `review_panels/stitch_contact_sheet.png`: state-blind review artifacts. Accepted review rows labeled `spatially_distributed_accepted` are selected from deterministic XYZ bins. No PDFs are emitted.
...
Important provenance fields in `track_stitch_edges.csv` include source/target IDs and labels, session/elapsed gaps, forward and reciprocal ranks/margins, transform source/QC, independent direct-vs-composed gap-2 projection deltas, feature-row safety flags, anchor support and residuals, history/future support, optional shape/matcher diagnostics, reason codes, assignment cost/component, and algorithm version. Unavailable optional measurements remain empty/NaN; missing feature rows or edge evidence fail closed.

## Benchmark guardrail

The benchmark splits trusted continuous canonical tracks into batched source/target pseudo-fragments, hides the intervening observations, and runs one global assignment for each replicate/gap batch through the same candidate, gate, and assignment code used on real endpoints. Trusted truth excludes pre-existing gaps, cycle/fallback warnings, edge ROIs, missing feature rows, and unreliable transforms. A canonical track/gap truth unit is sampled at most once across the full run; unique truth-track/split counts are reported and required by the write guard. It includes gaps 1, 2, and 3, deterministic samples, explicit no-successor controls (`no_successor_no_mask` and `no_successor_nonstart_mask`), target-only controls, and collision competitors. It reports precision, recall by gap, negative false-positive rate, collision outcomes, and one-sided Wilson intervals.
