...
from endpoint_evaluator import _read_csv, _session_table, load_matcher_spacing
from endpoint_stitcher import (
    ...
    build_synthetic_stitch_benchmark, edge_table, summarize_stitch_benchmark,
)
...
def _prepare_output(output_dir: Path, overwrite: bool) -> pd.DataFrame:
    ...
    known = {
        ...
        "track_stitch_edges.csv", "track_uid_stitch_map.csv", "tracks_graph_stitched.csv",
...
def _manual_manifest(proposals: pd.DataFrame, previous: pd.DataFrame) -> pd.DataFrame:
    columns = [
        "stitch_edge_id", "source_track_uid", "target_track_uid", "session_gap",
        "candidate_tier", "review_reasons", "manual_class", "manual_confidence", "reviewer_notes",
    ]
...
def run_endpoint_stitching(
    ...
    *,
    ...
    config = StitcherConfig(max_gap_sessions=max_gap_sessions, search_radius_um=search_radius_um)
    ...
    previous_manual = _prepare_output(output_dir, overwrite)
    ...
    benchmark = build_synthetic_stitch_benchmark(
        ...
        replicates=benchmark_replicates, random_seed=random_seed,
    )
    ...
    edge_table(assignments.loc[assignments.get("accepted_by_global_assignment", pd.Series(False, index=assignments.index))]).to_csv(output_dir / "track_stitch_edges.csv", index=False)
    ...
    manual = _manual_manifest(proposals, previous_manual)
    ...
    plot_stitch_review_panels(
        proposals, output_dir / "review_panels", max_panels=max_review_panels,
