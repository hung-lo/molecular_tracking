...
def main(argv=None):
    parser=argparse.ArgumentParser(); parser.add_argument("--project-config",required=True); parser.add_argument("--mouse-id",required=True); parser.add_argument("--laser-nm",type=int,default=1050); parser.add_argument("--catalog",default=None)
    ...
    path,ready=build_manifest_plan(config,rows,args.mouse_id,args.laser_nm); print(f"{'manifest' if ready else 'plan'}={path}")
    return 0
